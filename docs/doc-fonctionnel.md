...
| F22 | Client auto (démo) | Bouton du bandeau de la page coach : l'IA **lit l'historique de la conversation** et **propose la question suivante du client**, qui **remplit le champ de saisie** (jamais envoyée automatiquement — l'utilisateur relit, modifie puis envoie, ou pas). Même rôle que l'Agent C de l'atelier : le client ne conseille pas, n'invente aucun chiffre et ne connaît que les **trois chiffres de son dossier**. Nécessite une **IA réelle** (le mode démo est refusé avec un message explicite) |
