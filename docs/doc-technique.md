...
| GET | `/api/conversations/directory` | **Annuaire des conversations** (page Centre d'appels) : conversations clôturées + code client `AAA0000`, agence de gestion et score commercial + statut d'avancement. Paramètres `days` (5/10/30, `0` = tout), `category` (CREDIT_CONSO, CREDIT_IMMO, EPARGNE, ASSURANCE, AUTRE), `agency` (code agence à 5 chiffres), `status` (NOUVEAU, CONTACTE, QUALIFIE, RDV, CONCLU, PERDU, CLOTURE), `q` (client, agence, titre, projet, produit), `sort` (date, score, client, categorie, titre), `order` (asc, desc) |
